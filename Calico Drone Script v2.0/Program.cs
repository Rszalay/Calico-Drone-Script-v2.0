...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            switch (mod)
            {
                ...
                case 3:
                    ...
                    break;
                default:
                    Echo("How did you even manage this?");
                    break;
            }
        }
        ...
        public void FindObjectiveData()
        {
            Dictionary<MyDetectedEntityInfo, float> allDetected = new Dictionary<MyDetectedEntityInfo, float>();
            wcPbApi.GetSortedThreats(Me, allDetected);
            bool flag = false;
            foreach(var detectedEntity in allDetected)
            {
                if(detectedEntity.Key.EntityId == currentTarget.EntityId) { currentTarget = detectedEntity.Key; flag = true; }
            }
            if (flag)
            {
                target.Run(currentTarget.Position, myCurrentPosition);
            }
            else
            {
                target.Reset(engageRange);
                SelectTarget();
                FindObjectiveData();
            }
        }
        ...
        {
            myCurrentPosition = Me.WorldMatrix.Translation;
